...
int main() {
    ...
    // restoring edit sequence
    debugger.logMessage("Restoring edit sequence");

    // starting from right-bottom cell
    int i = a.size(), j = b.size();
    std::string sequence;

    while (i != 0 || j != 0)
    {
        // choosing minimal neighbour
        debugger.logMessage("Choosing min neighbour for", i, j);
        debugger.logNeighbours(distance, i, j);
        int min = std::min((i - 1 >= 0 && j - 1 >= 0) ? distance[i - 1][j - 1] : INT_MAX, 
                                std::min((i - 1 >= 0) ? distance[i - 1][j] : INT_MAX, 
                                        (j - 1 >= 0) ? distance[i][j - 1] : INT_MAX));
        
        debugger.logMessage("Min neighbour", min);
        
        
        // identifying chosen step
        debugger.logMessage("Identifying step");
        // Replace or Match
        if (i - 1 >= 0 && j - 1 >= 0 && distance[i - 1][j - 1] == min)
        {
            // if cost match -- match, else -- replace
            char step = (distance[i][j] == distance[i - 1][j - 1]) ? 'M' : 'R';
            debugger.logStep(step);

            sequence += step;
            i--;
            j--;

            debugger.logMessage("Current reversed sequence:");
            debugger.logMessage(sequence);

            continue;
        }
        // Delete
        if (i - 1 >= 0 && distance[i - 1][j] == min)
        {
            debugger.logStep('D');
    ...
    // reversing sequence because it was built from end to start
    std::reverse(sequence.begin(), sequence.end());
    debugger.logMessage("Sequence restored");
